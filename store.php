...
if (!isset($testmode)) {
  ...
  mysqli_free_result($result);
            ...
            $myCard = ['number' => $_POST['number'], 'exp_month' => $_POST['month'], 'exp_year' => $_POST['year']];
            $charge = \Stripe\Charge::create(['card' => $myCard, 'amount' => $cost, 'currency' => 'usd']);
        } catch (Exception $e) {
...
You have <?php echo count($_SESSION['cart']);
...
<a href="?pay&price=<?php echo urldecode($pricetag);
    ?> ">Pay <?php echo money_format('$%i', $pricetag);
    ?></a>
<?php
